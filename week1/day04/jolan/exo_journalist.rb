...
def array_str(array)
    nb = 0
    array.each do |elem|
        i = elem.include?("aude") || elem.include?("Aude")
        if i == true
            nb += 1
        end
    end
...
def array_upcase_contains(array)
    nb = 0
    array.each do |elem|
...
def array_underscore(array)
    nb = 0
    array.each { |elem| nb+=elem.scan(/[_]/).length }
    return nb
end

def array_sort_alpha(array)
    sort = array.sort_by!{ |elem| elem.downcase }
    return sort
end
